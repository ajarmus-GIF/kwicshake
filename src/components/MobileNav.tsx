  ...
  return (
    <>
      ...
      {/* Kept mounted rather than conditionally rendered so it can fade both ways. `inert` is what
          ...
          which a plain `opacity-0` would not achieve. `visibility` transitions alongside opacity
          so the panel stays painted for the length of the fade-out instead of vanishing at once. */}
      <div
        ref={panelRef}
        id={panelId}
        inert={!open}
      ...
      >
        <div
          className="absolute inset-0 bg-[var(--color-raised)]/95 backdrop-blur-md"
          onClick={() => close(true)}
        />
